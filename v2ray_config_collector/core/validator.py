...
class ConnectivityValidator:
    ...
    def extract_sing_box_core_id(self, config_text):
        ...
        try:
            ...
            # 2. VLESS / TROJAN (Вырезаем UUID пользователя между // и @)
            ...
            # 3. SHADOWSOCKS (Вырезаем уникальный зашифрованный ключ до @)
    ...
    def test_all_configs(self):
        ...
        print("🛡️ Модуль фильтрации ядра Sing-Box [Vless/Vmess/SS/Trojan] успешно внедрен.")
        ...
        # 1. Сначала прогоняем всю базу через ядро Sing-Box, вычищая одинаковые протоколы
        unique_by_core = []
        ...
        for cfg in raw_configs:
            ...
            if core_key in self.seen_cores:
                ...
                continue # Срезаем дубликат на входе, не тратя время на пинг порта
            self.seen_cores.add(core_key)
            unique_by_core.append(cfg)

        print(f"📡 Всего загружено: {len(raw_configs)} узлов.")
        print(f"🛡️ Движок Sing-Box заблокировал одинаковых протоколов: {duplicate_cores_count} шт.")
        print(f"⚡ Отправлено на TCP-валидацию портов: {len(unique_by_core)} уникальных серверов.")
        ...
        print(f"\n==========================================================================")
        print(f"✅ ВАЛИДАЦИЯ НА ЗАВОДЕ ЗАВЕРШЕНА!")
        print(f"🏆 Живых и уникальных серверов для твоей семьи: {len(valid_configs)}")
        print(f"🛡️ Всего отсеяно дубликатов ядра Sing-Box: {duplicate_cores_count}")
